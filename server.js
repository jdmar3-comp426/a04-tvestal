...
app.patch("/app/update/user/:id", (req, res) => {
	...
	res.status(200).json({"message": `${stmt["changes"]} record updated: ID ${stmt["lastInsertRowid"]} (200)`});
});

// DELETE a single user (HTTP method DELETE) at endpoint /app/delete/user/:id
app.delete("/app/delete/user/:id", (req, res) => {
	const stmt = db.prepare("DELETE FROM userinfo WHERE id = ?").get(req["params"]["id"]);
	res.status(200).json({"message": `${stmt["changes"]} record deleted: ID ${stmt["lastInsertRowid"]} (200)`});
});
